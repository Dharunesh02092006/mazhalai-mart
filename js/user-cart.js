...
async function addToUserCart(productId, productName, productPrice, productImage, quantity = 1, buttonElement = null) {
    try {
        // Check if user is logged in
        const authResponse = await fetch('auth/user_status.php', {
            method: 'GET',
            credentials: 'include'
        });
        ...
        let cartImage = productImage || 'images/placeholder.jpg';
        if (cartImage === 'undefined' || !cartImage) {
            cartImage = 'images/placeholder.jpg';
        }
        ...
        const cartData = {
            ...
            product_image: cartImage,
            quantity: parseInt(quantity)
        };
        
        const cartResponse = await fetch('api/user_cart.php', {
            method: 'POST',
            headers: {
        ...
        if (result.success) {
            ...
            // Update button text if button element was provided
            if (buttonElement) {
                buttonElement.textContent = result.action === 'updated' ? 'Updated!' : 'Added!';
    ...
    items.forEach(item => {
        ...
        cartHTML += `
            <div class="cart-item" data-product-id="${item.product_id}">
                <img src="${imageSrc}" alt="${item.product_name}" class="cart-item-image" onerror="this.src='images/placeholder.jpg'">
...
function updateCartCount() {
    ...
    .then(data => {
        if (data.authenticated) {
            // User is logged in - get cart count from API
            fetch('api/user_cart.php', {
                method: 'GET',
                credentials: 'include'
            })
            ...
            .then(result => {
                if (result.success && result.cart_items) {
                    const totalCount = result.cart_items.reduce((sum, item) => sum + item.quantity, 0);
            ...
            // User not logged in - get count from localStorage
