...
class OrderAPI {
    ...
    public function createOrder($orderData) {
        try {
            ...
            $itemQuery = "INSERT INTO order_items (order_id, product_id, product_name, quantity, price, total, product_image) 
                         VALUES (:order_id, :product_id, :product_name, :quantity, :price, :total, :product_image)";
            ...
            foreach ($orderData['items'] as $item) {
                ...
                } else if (strpos($productImage, 'images/') === false && strpos($productImage, 'admin/') === false) {
                    $productImage = 'images/' . $productImage;
                }
                
                $itemStmt->bindParam(':product_image', $productImage);
                $itemStmt->execute();
            }
        ...
        try {
            ...
            foreach ($orders as &$order) {
                $itemsQuery = "SELECT oi.product_id, oi.product_name, oi.quantity, oi.price, oi.total, 
                              COALESCE(oi.product_image, 'images/placeholder.jpg') as image
                              FROM order_items oi 
                ...
                // Fix image paths for items
                foreach ($items as &$item) {
                    $imagePath = $item['image'] ?? 'images/placeholder.jpg';
                    if (empty($imagePath) || $imagePath === 'undefined' || $imagePath === 'null') {
                        $imagePath = 'images/placeholder.jpg';
                    } else if (strpos($imagePath, 'uploads/') === 0) {
                        $imagePath = 'admin/' . $imagePath;
                    } else if (strpos($imagePath, 'images/') === false && strpos($imagePath, 'admin/') === false && strpos($imagePath, 'http') === false) {
