...
-- Create database
CREATE DATABASE IF NOT EXISTS dummy_database;
USE dummy_database;
...
INSERT INTO order_items (order_id, product_id, product_name, quantity, price, total, product_image) VALUES
(1, 1, 'Baby Lotion', 2, 180.00, 360.00, 'images/lotion.webp'),
(1, 2, 'Baby Shampoo', 1, 150.00, 150.00, 'images/shampoo.webp');
...
-- Migration Notes:
-- If updating from older schema, run these commands:
-- ALTER TABLE order_items ADD COLUMN product_image VARCHAR(500) DEFAULT NULL AFTER total;
-- ALTER TABLE user_cart ADD COLUMN product_id INT NOT NULL AFTER user_id;
-- ALTER TABLE user_cart CHANGE COLUMN price product_price DECIMAL(10, 2) NOT NULL;
-- ALTER TABLE user_cart ADD COLUMN product_image VARCHAR(500) DEFAULT NULL AFTER product_price;
-- UPDATE user_cart SET product_id = (SELECT id FROM products WHERE name = product_name LIMIT 1) WHERE product_id IS NULL;